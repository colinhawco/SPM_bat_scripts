function preprocess_spm12(func, t1, smooth)
% preprocess_spm12(func, t1, [smooth])
%
% Inouts: func: a series of 4D functional nii files (full path). If
...
for fdx = 1:size(func,1)
    ...
    for rdx = 1:size(v,1)
        % raw inuts, for motion correction
        p = [ff ',' num2str(rdx)];
        dp(rdx,1:length(p)) = p;
